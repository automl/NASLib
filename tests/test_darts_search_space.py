...
class DartsDrNASGrowOptimizer(unittest.TestCase):
...
class DartsPCDartsIntegrationTest(unittest.TestCase):
    ...
    def test_update(self):
        ...
        self.assertTrue(len(stats) == 4)
        if torch.cuda.is_available():
            self.assertAlmostEqual(stats[2].detach().cpu().numpy(), 2.8589, places=3)
            self.assertAlmostEqual(stats[3].detach().cpu().numpy(), 2.9394, places=3)
        else:
            self.assertAlmostEqual(stats[2].detach().cpu().numpy(), 2.7005, places=3)
            self.assertAlmostEqual(stats[3].detach().cpu().numpy(), 3.3087, places=3)
