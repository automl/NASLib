...
train_data = (x_data[:test_idx], y_data[:test_idx], info_data[:test_idx],
              times_data[:test_idx])
test_data = (x_data[test_idx:], y_data[test_idx:], info_data[test_idx:],
             times_data[test_idx:])
supported_search_spaces = {'nasbench201': NasBench201SearchSpace()}


class PredictorsTest(unittest.TestCase):
    def setUp(self):
        self.args = utils.parse_args(
            args=['--config-file', 'assets/test_predictor.yaml'])
        ...
        self.load_labeled = (
            True if self.config.search_space in ['darts', 'nlp'] else False)
    ...
    def test_RFPredictor(self):
        ...
        predictor_evaluator.adapt_search_space(self.search_space,
                                               load_labeled=self.load_labeled,
                                               dataset_api=self.dataset_api)
        predictor_evaluator.single_evaluate(
            train_data=train_data,
            test_data=test_data,
            fidelity=self.config.fidelity_single)

        self.assertAlmostEqual(predictor_evaluator.results[-1]['kendalltau'],
                               0.4666666666666,
                               places=3)
        self.assertAlmostEqual(predictor_evaluator.results[-1]['mae'],
                               4.7984154693486,
                               places=3)
        self.assertAlmostEqual(predictor_evaluator.results[-1]['rmse'],
                               5.400776348052,
                               places=3)
        self.assertAlmostEqual(predictor_evaluator.results[-1]['pearson'],
                               0.7369233227443,
                               places=3)
        self.assertAlmostEqual(predictor_evaluator.results[-1]['spearman'],
                               0.5428571428571,
                               places=3)
