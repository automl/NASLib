...
class Trainer(object):
    ...
    def search(self, resume_from="", summary_writer=None, after_epoch: Callable[[int], None] = None,
        ...
        #transforms object returned to allow adjusting data augmentation regularization
        if self.optimizer.using_step_function:
            self.train_queue, self.valid_queue, _, self.train_transforms, self.valid_transforms = self.build_search_dataloaders(
                self.config
            )
        if self.config.search.adjust_regularization:
            if start_epoch==0:
                self.weight_decay = self.config.search.weight_decay
                self.max_weight_decay = self.config.search.max_weight_decay

            elif  self.optimizer.op_optimizer.param_groups[0]['weight_decay'] != self.weight_decay:
                for param_group in self.optimizer.op_optimizer.param_groups:
                        param_group['weight_decay'] = self.weight_decay

        #drop path added while search also
        if self.config.search.drop_path:
            self.optimizer.update_edges(
                update_func=lambda edge: edge.data.set(
                    "op", DropPathWrapper(edge.data.op)
                ),
                scope=self.optimizer.OPTIMIZER_SCOPE,
        ...
        for e in range(start_epoch, self.epochs):
            ...
            #Adjusting regularization of drop path and cutout
            self._adjust_regularization(e)
            ...
            #Early stopping
            if self.config.search.early_stopping:
                if self.optimizer.la_tracker.stop_search:
                    logger.info(
                        f"Early stopping at {self.optimizer.la_tracker.stop_epoch}"
                    )
                    break

            #Adjust L2 regularization
            if self.config.search.adjust_regularization:
                final_iteration = round(np.log(self.max_weight_decay) / np.log(self.weight_decay), 1) == 1.
                if self.optimizer.la_tracker.stop_search and not final_iteration:
                    self.weight_decay *= self.config.search.mul_factor
                    start_again_epoch = self.optimizer.la_tracker.stop_epoch - self.config.search.extra_rollback_epochs
                    logger.info(
                        f"Adjusting weight decay to {self.weight_decay} and rolling back to {start_again_epoch} epoch."
                    )
                    self.search(start_again_epoch,summary_writer,after_epoch,report_incumbent)
                    break
    ...
    def build_search_dataloaders(config):
        train_queue, valid_queue, test_queue, train_transforms, valid_transforms = utils.get_train_val_loaders(
            config, mode="train"
        )
        return train_queue, valid_queue, test_queue, train_transforms, valid_transforms  # test_queue is not used in search currently
    ...
    def _adjust_regularization(self, e):
        drop_path_prob = self.config.evaluation.drop_path_prob * e / self.epochs
        self.optimizer.update_edges(
            update_func=lambda edge: edge.data.set(
                "drop_path_prob", drop_path_prob
            ),
        ...
        self.train_transform.transforms[-1].cutout_prob = self.config.cutout_prob * e / (self.epochs - 1)
        logging.info('drop_prob %e cutout_prob %e', drop_path_prob, self.train_transform.transforms[-1].cutout_prob)
