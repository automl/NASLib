...
class OmniSemiNASPredictor(Predictor):
    ...
    def fit(
        ...
        for i in range(iterations):
            ...
            # Pre-train
            print("Pre-train EPD")
            train_controller(self.model, xtrain_full_features, ytrain_normed,
                             pretrain_epochs)
            ...
            if self.semi and not self.add_lce:
                ...
                if up_sample_ratio is None:
                    up_sample_ratio = np.ceil(
                        m / len(xtrain_full_features)).astype(np.int)
                else:
                    up_sample_ratio = up_sample_ratio

                combined_input = (xtrain_full_features * up_sample_ratio +
                                  synthetic_full_features)
                combined_target = (list(ytrain_normed) * up_sample_ratio +
                                   synthetic_target)
                print("Train EPD")
                train_controller(self.model, combined_input, combined_target,
                                 epochs)
        ...
        if len(self.zero_cost) > 0:
            ...
            for method_name in self.zero_cost:
                if self.ss_type in ["nasbench101", "darts"]:
                    zc_method = ZeroCostV2(self.config,
                                           batch_size=64,
                ...
                self.xtrain_zc_info[f"{method_name}_scores"] = zc_method.query(
                    xtrain)
                self.xtest_zc_info[f"{method_name}_scores"] = zc_method.query(
                    xtest)
                if unlabeled is not None:
                    self.unlabeled_zc_info[
                        f"{method_name}_scores"] = zc_method.query(unlabeled)
