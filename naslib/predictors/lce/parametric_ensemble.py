...
class ParametricEnsemble:
    ...
    def fit(self, x, fit_weights=False):
        if fit_weights:
            ...
            for model in self.parametric_models:
                model.fit(x)
            self.params = {model.name: model.get_params() for model in self.parametric_models}
    ...
    def predict(self, x, params=None, weights=None):
        if params is not None:
            return sum([w * model.predict(x, params=params[model.name]) for (w, model) in zip(weights, self.parametric_models)])
        return sum([w * model.predict(x) for (w, model) in zip(self.weights, self.parametric_models)])
    ...
    def perturb_params(self, params, weights, sigma_sq, var):
        ...
        deg_freedom = 1 + len(self.weights) + sum([model.degrees_freedom for model in self.parametric_models])
        perturbation = np.random.normal(loc=0, scale=var, size=(deg_freedom,))
        perturbed_params = params.copy()
        perturbed_weights = weights.copy()
        pos = 0
        for model in self.parametric_models:
            perturbed_params[model.name] += np.concatenate([perturbation[pos:pos + model.degrees_freedom], np.zeros((1,))])
    ...
    def mcmc(self, x, N=10000, var=.0001, fit_weights=False, verbose=False):
        acceptances, stochastic_rejections, pathological_rejections, way_off_rejections = 0, 0, 0, 0
        ...
        for t in range(N):
            ...
            if verbose:
                if t == 1:
                    ...
                    last_power_two = t
                    print(f'Completed {t} Metropolis steps in {time.time() - start} seconds.')
            ...
            candidate_params, candidate_weights, candidate_sigma_sq = self.perturb_params(
                params, weights, sigma_sq, var)
            ...
            for j in range(curvelen):
                jth_error = self.predict(j + 1, params=candidate_params, weights=candidate_weights) - x[j]
                point_likelihood = norm.pdf(jth_error, scale=np.sqrt(candidate_sigma_sq))
            ...
            acceptance_probability = min(1, np.exp(candidate_log_likelihood - current_log_likelihood))
            if self.predict(curvelen + 1, params=candidate_params, weights=candidate_weights) > \
        ...
        print(f'Completed with acceptance rate {acceptances / N} in {time.time() - start} seconds.')

    def mcmc_sample_predict(self, x):
        return sum([self.predict(x, params=p[0], weights=p[1]) for p in self.mcmc_sample_params]) / len(self.mcmc_sample_params)
