...
class MovementOptimizer_test(MetaOptimizer):
    """
    ...
    def __init__(
        ...
        """
        super(MovementOptimizer_test, self).__init__()
        ...
        #self.path="/work/dlclarge2/agnihotr-ml/NASLib/naslib/optimizers/oneshot/movement/correct_search_prox_darts/warm_10_mask10_train_0.5/darts/cifar10/movement_test/14/search/scores.pth"
        try:
    ...
    def get_final_architecture(self):
        ...
        all_scores=torch.load(self.path)
        iter = iterator()

        def update_l2_weights2(edge):            
            """
        ...
        def load_scores(edge):
            if edge.data.has("score"):
                with torch.no_grad():
                    i = iter.get_data()
                    edge.data.score = all_scores[i]
                    print("\n\n\n\t\t\tLOADED SCORES\n\n\n")
                

        #temp=torch.tensor([1,3,8,8]).to(self.device)
        #self.graph(temp)
        #self.graph.update_edges(update_l2_weights2, scope=self.scope, private_edge_data=True) 
        ...
        #graph.update_edges(load_scores, scope=self.scope, private_edge_data=True)       
        """ scoring=torch.nn.ParameterList()
        ...
        logger.info(
            "In GET FINAL ARCHITECTURE: Group scores (importance scores, last column max): \n{}".format(
                "\n".join(weights_str)
            )
        ) """
...
#path=self.config.out_dir+'/'+self.config.search_space+'/'+self.config.dataset+'/'+self.config.optimizer+'/'+ str(self.config.seed)+'/search/scores.pth'