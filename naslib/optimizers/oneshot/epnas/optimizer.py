...
class EPNASOptimizer(MetaOptimizer):
    """
    ...
    def __init__(
        ...
        """
        super(EPNASOptimizer, self).__init__()
    ...
    def step(self, data_train, data_val, k=None):
        ...
        if unrolled:
            ...
            # EPNASOptimizer.mask = False
            # logits_train = self.graph(input_train)
            # train_loss_0 = self.loss(logits_train, target_train)
            logits_train = self.graph(input_train)
            train_loss = self.loss(logits_train, target_train)# + train_loss_0
            train_loss.backward()
            if self.grad_clip:
    ...
    def step_inner(self, data_train, data_val):
        input_train, target_train = data_train
        input_val, target_val = data_val
    ...
    def step_outer(self, data_train, data_val):
        input_train, target_train = data_train
        input_val, target_val = data_val
...
# only one op - randomly according to op one_hot_categorical softmax probs
class GetSubnet(autograd.Function):
    @staticmethod
    def forward(ctx, scores, k):
        # Get the supermask by sorting the scores and using the top k%
...
# # only one op - randomly according to op one_hot_categorical ... 1 probs
# class GetSubnet(autograd.Function):
#     @staticmethod
#     def forward(ctx, scores, k):
#         # Get the supermask by sorting the scores and using the top k%
#         out = scores.clone() * 100
#         # _, idx = scores.flatten().sort()

#         out_positive = (F.elu(out) + 1)
#         probs = out_positive / out_positive.sum(dim=-1)
#         print('probs =', probs)
#         m = torch.distributions.one_hot_categorical.OneHotCategorical(probs)
#         sampled = m.sample()
...
# # top k ops - randomly according to op dirichlet probs
# class GetSubnet(autograd.Function):
#     @staticmethod
#     def forward(ctx, scores, k):
#         # Get the supermask by sorting the scores and using the top k%
#         out = scores.clone()
#         betas = torch.nn.functional.elu(out) + 1

#         m = torch.distributions.dirichlet.Dirichlet(betas)
#         sampled = m.sample()

#         _, idx = sampled.flatten().sort()
#         j = int((1 - k) * sampled.numel())
...
#     def forward(self, x, edge_data):
#         sparsity = (1 - EPNASOptimizer.k / 49) * (1 - 1 / len(edge_data.alpha)) + (EPNASOptimizer.k / 49) * 1 / len(edge_data.alpha)
