...
#SBATCH -J nb201-re           # search space - algorithm
...
for seed in {1..4}; do
  ...
  if [ $gpu_counter -eq $SLURM_ARRAY_TASK_ID ]; then
    ...
    #sleep 1
    python runner.py --config-file config_cifar10.yaml --seed ${seed} --optimizer re search.checkpoint_freq 100 search.epochs 1000
    exit $?
  fi
  let gpu_counter+=1
done
