...
class Compose(T.Compose):
    def __init__(self, task_name, transforms):
        ...
        assert task_name in TASK_NAMES, "task_name must be one of {}".format(
            TASK_NAMES)
...
class Resize(T.Resize):
    def __init__(self,
        ...
        assert isinstance(input_size,
                          int) or (isinstance(input_size, Iterable)
                                   and len(input_size) == 2)
        if target_size:
            assert isinstance(target_size,
                              int) or (isinstance(target_size, Iterable)
    ...
    def __call__(self, sample, task_name):
        ...
        if task_name in ['autoencoder', 'normal']:
            ...
            return {
                ...
                'class_object', 'class_scene', 'room_layout', 'jigsaw'
        ]:
            return {
                'image': F.resize(image, self.input_size, self.interpolation),
                'label': label,
    ...
    def __call__(self, sample, task_name):
        ...
        if task_name in ['autoencoder', 'normal']:
            ...
            return {
                ...
                'class_scene', 'class_object', 'jigsaw', 'room_layout'
        ]:
...
class Normalize(T.Normalize):
    ...
    def __call__(self, sample, task_name):
        ...
        if task_name in ['autoencoder']:
            return {
                'image': F.normalize(tensor, self.mean, self.std,
                                     self.inplace),
...
class ColorJitter(T.ColorJitter):
    ...
    def __call__(self, sample, task_name):
        ...
        t = self.get_params(self.brightness, self.contrast, self.saturation,
                            self.hue)
...
def load_class_object_logits(label_path,
                             selected=False,
                             normalize=True,
                             final5k=False):
    try:
    ...
    if selected:
        selection_file = os.path.join(
            lib_data_dir, "data",
            "class_object_final5k.npy") if final5k else os.path.join(
                lib_data_dir, "data", "class_object_selected.npy")
...
def load_class_object_label(label_path, selected=False, final5k=False):
    logits = load_class_object_logits(label_path,
                                      selected=selected,
                                      normalize=True,
                                      final5k=final5k)
...
def load_class_scene_logits(label_path,
                            selected=False,
                            normalize=True,
                            final5k=False):
    try:
    ...
    if selected:
        selection_file = os.path.join(
            lib_data_dir, "data",
            "class_scene_final5k.npy") if final5k else os.path.join(
                lib_data_dir, "data", "class_scene_selected.npy")
        selection = np.load(selection_file)
        logits = logits[selection.astype(bool)]
        if normalize:
            logits = logits / logits.sum()
...
def image2tiles(img, permutation, new_dims):
    ...
    if len(permutation) != 9:
        raise ValueError(
            f'Target permutation of Jigsaw is supposed to have length 9, getting {len(permutation)} here'
        )
...
class MakeJigsawPuzzle(object):
    ...
    def __call__(self, sample, task_name):
        ...
        if task_name == 'jigsaw':
            ...
            if self.centercrop:
                image = F.center_crop(
                    image, (He * self.centercrop, Wi * self.centercrop))
