    ...
    try:
        for arg, value in pairwise(args.opts):
            if "." in arg:
                ...
                config[arg] = type(
                    config[arg])(value) if arg in config else value
...
def get_train_val_loaders(config, mode):
    ...
    if dataset == "cifar10":
        ...
        train_data = dset.CIFAR100(root=data,
                                   train=True,
                                   download=True,
                                   transform=train_transform)
        test_data = dset.CIFAR100(root=data,
                                  train=False,
                                  download=True,
                                  transform=valid_transform)
    elif dataset == "svhn":
        train_transform, valid_transform = _data_transforms_svhn(config)
        train_data = dset.SVHN(root=data,
                               split="train",
                               download=True,
                               transform=train_transform)
        test_data = dset.SVHN(root=data,
                              split="test",
                              download=True,
                              transform=valid_transform)
...
def get_jigsaw_configs():
    ...
    cfg['dataset_dir'] = os.path.join(get_project_root(), "data",
                                      "taskonomydata_mini")
    cfg['data_split_dir'] = os.path.join(get_project_root(), "data",
                                         "final5K_splits")
    ...
    cfg['train_transform_fn'] = load_ops.Compose(cfg['task_name'], [
        ...
        load_ops.MakeJigsawPuzzle(classes=cfg['target_dim'],
                                  mode='max',
                                  tile_dim=(64, 64),
                                  centercrop=0.9,
                                  norm=False,
...
def get_class_object_configs():
    ...
    cfg['target_load_kwargs'] = {
        'selected':
        True if cfg['target_dim'] < 1000 else False,
        'final5k':
        True if cfg['data_split_dir'].split('/')[-1] == 'final5k' else False
    }
...
def get_class_scene_configs():
    ...
    cfg['target_load_kwargs'] = {
        'selected':
        True if cfg['target_dim'] < 365 else False,
        'final5k':
        True if cfg['data_split_dir'].split('/')[-1] == 'final5k' else False
    }
...
def get_last_checkpoint(config, search=True):
    ...
    """
    try:
        path = os.path.join(config.save, "search" if search else "eval",
                            "last_checkpoint")
        with open(path, "r") as f:
            checkpoint_name = f.readline()
        return os.path.join(config.save, "search" if search else "eval",
                            checkpoint_name)
    except:
        return ""
...
def cross_validation(xtrain,
                     ytrain,
                     predictor,
                     split_indices,
                     score_metric="kendalltau"):
    validation_score = []
    ...
    for train_indices, validation_indices in split_indices:
        ...
        if score_metric == "pearson":
            ...
            score_i = metrics.mean_squared_error(yval_i,
                                                 ypred_i,
                                                 squared=False)
            ...
            score_i = stats.kendalltau(yval_i, np.round(ypred_i,
                                                        decimals=2))[0]
...
class Checkpointer(fvCheckpointer):
    def load(self, path: str,
        ...
        if (incompatible is
                not None):  # handle some existing subclasses that returns None
