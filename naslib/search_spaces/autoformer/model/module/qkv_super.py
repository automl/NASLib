...
class Proj_emb_choice(AbstractPrimitive):
    ...
    def set_sample_config(self):
        self.proj.sample_weight = self.proj.sample_weight[:self.sampled_in_dim, :]
    ...
    def forward(self, x, edge_data):
        ...
        x = F.linear(x[:,:,x.sum(dim=(0,1)) != 0], self.proj.sample_weight, self.proj.sample_bias) * (
            self.sample_scale if self.proj.scale else 1)
...
class QKV_super_embed_choice(AbstractPrimitive):
    def __init__(self, qkv_super, embed_choice, super_emb, pre_norm):
    ...
    def set_sample_config(self):
        self.qkv_super.sample_weight = self.qkv_super.weight[:, :self.
                                                             sampled_in_dim]
        self.attn_layer_norm.set_sample_config(sample_embed_dim=self.sampled_in_dim)
    ...
    def forward(self, x, edge_data):
        self.set_sample_config()
        x = self.maybe_layer_norm(self.attn_layer_norm, x[:,:,:self.sampled_in_dim], before=True)
        return x
...
class QKV_super_head_choice(AbstractPrimitive):
    def __init__(self, qkv_super, rel_pos_embed_k, rel_pos_embed_v, proj, head_choice, attn_drop, super_emb, super_head_emb_dim):
    ...
    def set_sample_config(self):
        ...
        self.proj.sample_weight = self.proj.weight[:, :64*self.sample_num_heads]

    def forward(self, x, edge_data):
        self.set_sample_config()
        B, N, C = x.shape
        print("QKV in shape", x.shape)
        qkv = self.qkv_super(x).reshape(B, N, 3, self.sample_num_heads, -1).permute(2, 0, 3, 1, 4)
        ...
        output = torch.zeros(
            [x.shape[0], x.shape[1], self.super_head_emb_dim])
